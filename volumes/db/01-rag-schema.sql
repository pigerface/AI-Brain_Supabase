...
-- • 🛡️ 安全強化：所有函數添加 SET search_path = 'public, extensions' 防止注入攻擊
-- • 使用 EXTRACT() 取代 to_char() 確保 generated column 的 immutability
-- ============================================================

-- ---- Extensions ------------------------------------------------------------
CREATE EXTENSION IF NOT EXISTS pgcrypto;  -- 用於 gen_random_uuid() 和 digest()
CREATE EXTENSION IF NOT EXISTS "uuid-ossp";  -- 若仍需要 v4/v5 其他地方使用
...
CREATE TABLE IF NOT EXISTS resources (
  ...
  content_header    text NOT NULL,                   -- 內容標題（必填，用於生成 fingerprint）
  ...
  fingerprint       text GENERATED ALWAYS AS (
    encode(
      digest(
        COALESCE(source_id,'') || ':' ||
        COALESCE(content_header,'') || ':' ||
        EXTRACT(YEAR FROM content_time AT TIME ZONE 'UTC')::text ||
        LPAD(EXTRACT(MONTH FROM content_time AT TIME ZONE 'UTC')::text, 2, '0'),
        'sha256'
      ),
      'hex'
    )
...
-- 業務唯一性約束
CREATE UNIQUE INDEX IF NOT EXISTS uq_resources_fingerprint ON resources(fingerprint);
...
-- 基本政策：允許認證用戶讀取所有資料
DROP POLICY IF EXISTS "Allow authenticated users to read media_sources" ON media_sources;
CREATE POLICY "Allow authenticated users to read media_sources" ON media_sources
    FOR SELECT USING (auth.role() = 'authenticated');

DROP POLICY IF EXISTS "Allow authenticated users to read resources" ON resources;
CREATE POLICY "Allow authenticated users to read resources" ON resources
    FOR SELECT USING (auth.role() = 'authenticated');

DROP POLICY IF EXISTS "Allow authenticated users to read parsed_artifacts" ON parsed_artifacts;
CREATE POLICY "Allow authenticated users to read parsed_artifacts" ON parsed_artifacts
    FOR SELECT USING (auth.role() = 'authenticated');

DROP POLICY IF EXISTS "Allow authenticated users to read images" ON images;
CREATE POLICY "Allow authenticated users to read images" ON images
    FOR SELECT USING (auth.role() = 'authenticated');

DROP POLICY IF EXISTS "Allow authenticated users to read chunks" ON chunks;
CREATE POLICY "Allow authenticated users to read chunks" ON chunks
    FOR SELECT USING (auth.role() = 'authenticated');

DROP POLICY IF EXISTS "Allow authenticated users to read chunk_embeddings" ON chunk_embeddings;
CREATE POLICY "Allow authenticated users to read chunk_embeddings" ON chunk_embeddings
    FOR SELECT USING (auth.role() = 'authenticated');

-- Service role 完整權限
DROP POLICY IF EXISTS "Service role full access to media_sources" ON media_sources;
CREATE POLICY "Service role full access to media_sources" ON media_sources
    FOR ALL USING (auth.role() = 'service_role');

DROP POLICY IF EXISTS "Service role full access to resources" ON resources;
CREATE POLICY "Service role full access to resources" ON resources
    FOR ALL USING (auth.role() = 'service_role');

DROP POLICY IF EXISTS "Service role full access to parsed_artifacts" ON parsed_artifacts;
CREATE POLICY "Service role full access to parsed_artifacts" ON parsed_artifacts
    FOR ALL USING (auth.role() = 'service_role');

DROP POLICY IF EXISTS "Service role full access to images" ON images;
...
DROP POLICY IF EXISTS "Service role full access to chunk_embeddings" ON chunk_embeddings;
CREATE POLICY "Service role full access to chunk_embeddings" ON chunk_embeddings
    FOR ALL USING (auth.role() = 'service_role');

-- =============================================================================
-- 7) 實用函數
-- 🛡️ 安全：所有函數使用 SET search_path = 'public, extensions' 防止注入攻擊
-- =============================================================================
...
AS $$
    SELECT 
        ...
        c.chunking_text,
        c.description,
        1 - (c.chunk_embedding OPERATOR(extensions.<=>) query_embedding) as similarity
    FROM public.chunks c
    WHERE c.chunk_embedding IS NOT NULL
        AND 1 - (c.chunk_embedding OPERATOR(extensions.<=>) query_embedding) > match_threshold
        AND (filter_source_id IS NULL OR c.source_id = filter_source_id)
    ORDER BY c.chunk_embedding OPERATOR(extensions.<=>) query_embedding
    LIMIT match_count;
$$;
...
AS $$
    ...
    vector_search AS (
        SELECT 
            ...
            c.chunking_text,
            c.description,
            1 - (c.chunk_embedding OPERATOR(extensions.<=>) query_embedding) as vector_score
        FROM public.chunks c
        WHERE c.chunk_embedding IS NOT NULL
            AND (filter_source_id IS NULL OR c.source_id = filter_source_id)
    )
...
LEFT JOIN public.resources r ON r.source_id = ms.id
LEFT JOIN public.parsed_artifacts p ON p.source_id = ms.id
LEFT JOIN public.chunks c ON c.source_id = ms.id
...
DO $$
DECLARE
    sample_resource_uuid uuid;
BEGIN
    -- 插入範例 media_sources
    INSERT INTO media_sources (id, name, description) VALUES ('example', 'Example Source', 'Sample media source for testing') ON CONFLICT DO NOTHING;
    
    -- 插入範例 resource (使用正確的欄位名稱)
    ...
    -- 取得插入的 resource UUID 用於範例 chunk
    SELECT uuid INTO sample_resource_uuid 
    FROM resources 
    WHERE remote_src_url = 'https://example.com/sample-article';
    
    IF sample_resource_uuid IS NOT NULL THEN
        INSERT INTO chunks (
            resource_uuid,
            chunk_order,
            chunking_text,
